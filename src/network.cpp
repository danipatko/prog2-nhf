  ...
  for (int from = 0; from < graph.vtx().size(); from++) {
    for (int to : graph.adjacent(from)) {
      if (from < to) {
        ...
        map.geo_roads.add(transform(node_from), transform(node_to),
                          motor2color(node_from.road->highway),
                          node_from.road->visibility());
      }
    }
  }
...
bool Network::trace(Algorithm<Node>::Trace &trace, const int spread_rate) {
  ...
  for (int i = 0; i < spread_rate; i++) {
    while (trace.has_next()) {
      ...
      map.discovered.add(p1, p2, color::ORANGERED,
                         graph.at(trace.current()).road->rating() * 2);
    }
...
void Network::run(Algorithm<Node> &algo, const std::vector<int> &path,
...
void parse(const std::string &filename, std::vector<Road *> &roads) {
  ...
  while (std::getline(file, line)) {
    ...
    if (roads.size() % 1000 == 0) {
      ...
      std::cout << "read " << std::setw(6) << roads.size() << " records in "
                << std::setprecision(6) << el
                << "ms \tbatch average: " << std::setprecision(6)
                << roads.size() / el * 1000 << " records/sec\n";
    }
  }
...
DiGraph<Node> construct_graph(std::vector<Vertex<Node>> &vlist,
  ...
  if (options.graph == DiGraph<Node>::Driver::Matrix) {
    std::cout << "You appear to have chosen the adjacency matrix graph driver. "
              ...
              << "The graph contains " << vlist.size()
              << " vertices, and the matrix size is about to be: "
              << vlist.size() * vlist.size() * sizeof(int) / 1024.f / 1024.f
              << " MBs\n"
...
DiGraph<Node> construct(const std::vector<Road *> &roads,
                        const cli::Options &options) {
